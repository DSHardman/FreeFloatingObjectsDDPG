% for Bayesian tests included in first paper
...
function cost = i180CostFunction(x)%, filename, cam,cameraParams,worldcentre,imagecentre)
    ...
    mindist = NaN;
    %[xdes, ydes] = pol2cart(5*pi/4,165); % 180deg Bayesian
    %[xdes, ydes] = pol2cart(pi/4,165); % 0deg Bayesian
    [xdes, ydes] = pol2cart(3*pi/4,165); % 90deg Bayesian
